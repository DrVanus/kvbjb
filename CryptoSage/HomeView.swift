        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            LazyVGrid(columns: columns, alignment: .leading, spacing: 12) {
                statCell(
                    ...
                    value: homeVM.marketVM.globalData?.totalMarketCap["usd"]
                        .map { $0.formatted(.currency(code: "USD")) } ?? "$0",
                    icon: "globe"
                )
                statCell(
                    title: "24h Volume",
                    value: homeVM.marketVM.globalData?.totalVolume["usd"]
                        .map { $0.formatted(.currency(code: "USD")) } ?? "$0",
                    icon: "clock"
                )
                statCell(
                    title: "BTC Dominance",
                    value: homeVM.marketVM.globalData?.marketCapPercentage["btc"]
                        .map { String(format: "%.2f%%", $0) } ?? "0.00%",
                    icon: "bitcoinsign.circle"
                )
                statCell(
                    title: "ETH Dominance",
                    value: homeVM.marketVM.globalData?.marketCapPercentage["eth"]
                        .map { String(format: "%.2f%%", $0) } ?? "0.00%",
