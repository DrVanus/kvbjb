...
final class MarketViewModel: ObservableObject {
    ...
    func fetchAllData() async {
        async let coinsTask = fetchCoins()
        async let globalTask = fetchGlobalData()
        _ = await (coinsTask, globalTask)
    }
    ...
    func fetchCoins() async {
        ...
        do {
            ...
            // Build a request with a non-blank User-Agent (Cloudflare often blocks empty-UA requests)
            var request = URLRequest(url: url)
            request.setValue("CryptoSageAI/1.0", forHTTPHeaderField: "User-Agent")
    ...
    /// Fetches global market summary directly from CoinGecko API
    func fetchGlobalData() async {
        ...
        do {
            ...
            let fetchedGlobal = try jsonDecoder.decode(GlobalMarketData.self, from: data)
            globalData = fetchedGlobal
            print("✅ fetchGlobalData loaded:", fetchedGlobal)
        } catch {
