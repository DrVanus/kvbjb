...
struct MarketView: View {
    @StateObject private var marketVM: MarketViewModel = .shared
                        ...
                        HStack(spacing: 24) {
                            ...
                            VStack(alignment: .leading) {
                                Text("24h Volume")
                                    .font(.caption2)
                                    .foregroundColor(.white.opacity(0.7))
                                Text(global.totalVolume.usd.formattedWithAbbreviations())
                                    .font(.headline)
                                    .foregroundColor(.white)
                            }
                            Spacer()
                            VStack(alignment: .leading) {
                                Text("BTC Dominance")
                                    .font(.caption2)
                                    .foregroundColor(.white.opacity(0.7))
                                Text(String(format: "%.1f%%", global.marketCapPercentage.btc))
                                    .font(.headline)
                                    .foregroundColor(.white)
                            }
                        }
    ...
    private var coinList: some View {
        ScrollView(showsIndicators: false) {
            LazyVStack(spacing: 0) {
                ForEach(marketVM.filteredCoins) { coin in
...
struct MarketView_Previews: PreviewProvider {
    static var marketVM = MarketViewModel()
