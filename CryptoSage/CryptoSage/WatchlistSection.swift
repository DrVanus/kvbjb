...
struct WatchlistSectionView: View {
    ...
    // Use dedicated watchlistCoins array loaded via API
    ...
    var body: some View {
        ...
        // Auto-refresh the data every 15 seconds (updating only price changes)
        .onReceive(refreshTimer) { _ in
            Task {
                await marketVM.loadWatchlist(ids: Array(marketVM.favoriteIDs))
            }
        }
        .onAppear {
            Task {
        ...
        .onChange(of: marketVM.favoriteIDs) { newIDs in
            Task {
                await marketVM.loadWatchlist(ids: Array(newIDs))
            }
        }
    }
    ...
    private func moveCoinInWatchlist(from source: IndexSet, to destination: Int) {
        // No reordering logic needed for global coin list
    }
