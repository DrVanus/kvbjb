...
final class CryptoAPIService {
    ...
    func fetchCoinMarkets() async throws -> [MarketCoin] {
        let url = URL(string: "https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=20&page=1&sparkline=true&price_change_percentage=24h")!
        var attempts = 0
        var lastError: Error?
        while attempts < 2 {
            do {
